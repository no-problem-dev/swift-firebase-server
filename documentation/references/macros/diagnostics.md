...
マクロ実装でのエラー報告と診断メッセージの実装について説明する。
...
> **重要**: マクロが不正なSwiftコードを生成してコンパイラにエラーを任せるよりも、マクロ自身が分かりやすいエラーメッセージを出力すべきだ。（参考: [Point-Free MacroTesting](https://github.com/pointfreeco/swift-macro-testing)）

## 例外をスロー

最もシンプルな方法。エラーはマクロ属性の位置に表示される。
